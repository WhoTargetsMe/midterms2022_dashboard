...
2. Advertised in the last 30 days (Nov 2nd - 1st December 2022) and spent money in Georgia.
...
runoff_spend_reps %>% 
  ...
  labs(x = "", y = "\nBudget spent on targeting method (% of Total spend)", caption = "Source: Meta Ad Library using data compiled by Who Targets Me. Data Viz: Fabio Votta (@favstats).")#,
       ...
       # subtitle = "Period: Nov 25th - 1st December 2022 (Last 7 Days)")
...
contested_dat %>% 
  ...
  left_join(lab_dat) %>% 
  mutate(value = factor(value, the_order)) %>% 
  mutate(page_name = factor(page_name, c("All Republican-supporting pagess", "All Democrat-supporting pages"))) %>% 
  # filter()
  ggplot(aes(value, perc)) +
  geom_col(aes(fill = page_name), position = position_stack(), alpha = 0.8) +
  coord_flip() +
